...
function getMap2(url){
    let dataBase = getData(url); 
    dataBase["features"] = dataBase["features"].slice(0,3)
    let shapeBase = L.geoJson(dataBase,{pointToLayer: getIconoAuxiliar}).addTo(map);
    return shapeBase;
}
...
function getMapaCustomStyle(url, estiloDinamico){
    let dataBase = getData(url);
    console.log(url,dataBase["features"].length)
    dataBase["features"] = dataBase["features"].slice(0,10000)
    let shapeBase = L.geoJson(dataBase,{
        style:estiloDinamico,
        onEachFeature: onEachFeature,
        pointToLayer: getIconoAuxiliar            
