...
class UTIL {    
    ...
    url_ícono
    */
    static setCapa2(capa,controlGlobalCapa,codComuna) {
        $.get({
            url: capa.urlData,
...
class COMUNABASE{
    constructor(codComuna){
        ...
        var DatosDirectos = comunasTest.map(x =>{
            this.urlBaseComuna = `https://raw.githubusercontent.com/Sud-Austral/DATA_MAPA_PUBLIC_V2/main/chile_comunas/${x}.json`;
            //Get Data de Comuna
            return getData(this.urlBaseComuna);
        })
        //console.log(DatosDirectos)
        var auxData = {"type": 'FeatureCollection', "features" : []};
        
        DatosDirectos.forEach(x => {
            //console.log(1,x["features"]);
            auxData["features"] = [...x["features"],...auxData["features"]];
            //console.log(2,auxData["features"])
        
        });
        
        this.dataBaseComuna = auxData;
        console.log("Hola",this.dataBaseComuna)
        ...
        nombreComuna = `<span class="comunaID"> Área de Estudio </span>`.toString();
        this.jsonComuna = {};
    ...
    constructor(comunaBase,controlGlobalCapa){          
        ...
        this.dataGlobalNivel1 = dataCapaGlobal.map(capa => {
            ...
            try {
                ...
                url_ícono
                */
                UTIL.setCapa2(capa,controlGlobalCapa,comunaBase.codigo_comuna); 
            } catch (error) {
