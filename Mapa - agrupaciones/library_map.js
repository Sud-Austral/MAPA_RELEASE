    ...
    constructor(nombre, url, funcionEstilo, pointToLayer) {
      ...
      if (this.data && this.data.features[0].geometry.type == "Point"){
        ...
        var markers = L.markerClusterGroup({ iconCreateFunction:
            function (cluster) { var markers = cluster.getAllChildMarkers(); return L.divIcon({ html: markers.length, className: 'mycluster', iconSize: L.point(40, 40) }); } });
        markers.addLayer(this.shape);
        console.log(markers,this.nombre);
        this.shape = markers;
      }  
      else{
