...
def main():
    ...
    try:
        shutil.rmtree("publicaciones3")
    except:
        pass

    try:
        os.mkdir("publicaciones3")
    except:
        pass
    ...
    for i,j in df.iterrows():
        ...
        print(2,i)
        with open(f"publicaciones3/{j['nombre']}/db/dataGlobal1.json", 'w', encoding='utf-8') as file:
            ref1.to_json(file,orient="records",force_ascii=False)

        with open(f"publicaciones3/{j['nombre']}/db/dataCapa1.json", 'w', encoding='utf-8') as file:
            ref2.to_json(file,orient="records",force_ascii=False)
        print(3,i)
        f = open (f"publicaciones3/{j['nombre']}/Index.html",'r')
        contenido = f.read()
        f.close()
        print(4,i)
        with open(f"publicaciones3/{j['nombre']}/Index.html", 'w', encoding='utf-8') as file:
